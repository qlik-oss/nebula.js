  ...
  async addDimensions(dimensions, alternative = false) {
    ...
    await Promise.all(
      dimensions.map(async (dimension) => {
        if (hypercubeUtils.isTotalDimensionsExceeded(this, existingDimensions)) {
          return addedDimensions;
        }

        const dim = initializeField(dimension);

        if (hypercubeUtils.isDimensionAlternative(this, alternative)) {
          const altDim = await hypercubeUtils.addAlternativeDimension(this, dim);
          addedDimensions.push(altDim);
        } else if (existingDimensions.length < this.maxDimensions()) {
          await hypercubeUtils.addActiveDimension(this, dim, existingDimensions, addedDimensions, addedActive);
          addedActive++;
        }
        return addedDimensions;
      })
