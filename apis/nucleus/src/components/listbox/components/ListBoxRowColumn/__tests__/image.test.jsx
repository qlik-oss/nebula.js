...
describe('<Image />', () => {
  ...
  test('should not render an img when src is missing (e.g. url mode before a url is provided)', async () => {
    const testRenderer = await render(
      <Image representation={{ imageSize: 'fitHeight', imagePosition: 'top-left' }} src={undefined} label="my label" />
    );
    const imgs = testRenderer.root.findAllByType('img');
  ...
  test('should not render an img when src is an empty string', async () => {
    const testRenderer = await render(
      <Image representation={{ imageSize: 'fitHeight', imagePosition: 'top-left' }} src="" label="my label" />
    );
    const imgs = testRenderer.root.findAllByType('img');
    expect(imgs).toHaveLength(0);
    await testRenderer.unmount();
  ...
  test('should let the container fill height for non-fitHeight sizes', async () => {
    const testRenderer = await render(
      <Image
        representation={{ imageSize: 'alwaysFit', imagePosition: 'top-left' }}
        src="http://foo/bar.png"
        label="l"
      />
    );
  ...
  describe('title overlay', () => {
    ...
    test('titleBackground=false removes the text background', async () => {
      const testRenderer = await render(
        <Image
          representation={{ imageSize: 'alwaysFill', imagePosition: 'top-center', titleBackground: false }}
          src="http://foo/bar.png"
          label="alt"
          title="t"
        />
  ...
  describe('corner radius and border', () => {
    test.each([
      ...
      const testRenderer = await render(
        <Image
          representation={{ imageSize: 'alwaysFill', imagePosition: 'top-center', cornerRadius }}
          src="http://foo/bar.png"
          label="l"
        />
      );
