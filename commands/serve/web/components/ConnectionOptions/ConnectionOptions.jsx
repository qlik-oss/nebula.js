...
export const ConnectionOptions = ({ info, onKeyDown }) => {
    ...
    <Box sx={{ width: '100%' }}>
      ...
      {OptionsToConnect.map(({ id, formFields }) => (
        <TabPanel tabIdx={tabIdx} idx={id} key={id}>
          <FormManager info={info} handleSubmit={onKeyDown} fields={formFields} />
        </TabPanel>
      ))}
    </Box>
  );
...
function TabPanel({ children, tabIdx, idx }) {
  if (tabIdx === idx) return <Box sx={{ p: 3 }}>{children}</Box>;
  else return null;
}
