...
export default function ({
  ...
  const snapIt = useCallback((doExport = false) => {
    if (!vizRef.current) {
      return;
    }
    if (doExport) {
      ...
      <Grid container direction="column" style={{ height: '100%', position: 'relative' }}>
        <Grid item>
          <Toolbar variant="dense" disableGutters style={{ padding: '0 8px' }}>
            ...
            <IconButton disabled={!model} title="Open in single render view" href={model ? `${document.location.href.replace(/\/dev\//, '/render/')}?object=${model.id}&permissions=passive,interact,select&theme=${currentThemeName}` : ''} target="_blank">
              {SvgIcon({
                size: 'large',
                viewBox: '0 0 24 24',
